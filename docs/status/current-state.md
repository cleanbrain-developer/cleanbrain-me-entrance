...
- Retrofitted `agent-dev-starter`'s V2 migration onto open standards (`ADR-0010` through `ADR-0013` in `agent-dev-starter`), 2026-09-20:
  ...
  - Deliberately not done at the time: actually running `specify init` / installing the pinned Spec Kit CLI (`specify-cli==1.0.8`) — closed below (2026-09-20).
- Installed the real, pinned Spec Kit CLI (`specify-cli==1.0.8`, via a working Python 3.12 + `uv` toolchain) and actually ran it in this repository, 2026-09-20:
  - `specify init --here --force --ignore-agent-tools --integration claude --script sh --non-interactive` generated the real `.specify/templates/`, `.specify/scripts/`, `.specify/workflows/` and `.claude/skills/speckit-*/SKILL.md`; `specify integration install codex --force` added the matching `.agents/skills/speckit-*/SKILL.md`. Both were hand-listed in `PROJECT.yaml`'s `supported_agents.initial` and are now real CLI-rendered files, not hand-written.
  - `specify init` recognized the hand-drafted `.specify/memory/constitution.md` from the earlier migration as already customized ("Constitution setup (existing file preserved)") and left it untouched — verified byte-for-byte identical (`git diff` empty) before and after the install. It still holds this project's real principles, not the generic placeholder template.
  - Copied `scripts/check-ko-companions.sh` from `agent-dev-starter` (`ADR-0014`): it now excludes vendored Spec Kit assets (`.specify/templates|scripts|workflows|integrations`, any `.claude/skills/speckit-*`/`.agents/skills/speckit-*`) from the `.ko.md` mandate, since that content is upstream-authored and replaced wholesale on every `specify upgrade`. `.specify/memory/constitution.md` remains mandatory and already had a `.ko.md` companion. `scripts/check-ko-companions.sh --missing-only` passes clean (0 missing).
  - This closes the gap recorded below in "Known constraints" as of the previous update.
...
1. Ordinary feature/content work from here — no remaining foundation, deployment-pipeline, or Spec Kit tooling gaps.
...
- (Resolved 2026-09-20) The GitHub Spec Kit CLI (`specify-cli==1.0.8`, pinned in `PROJECT.yaml`'s `standards.spec_kit.pinned_version`) is now actually installed and has been run in this repository — real `.specify/templates/`, `.specify/scripts/`, `.specify/workflows/`, `.claude/skills/speckit-*/SKILL.md`, and `.agents/skills/speckit-*/SKILL.md` exist, generated by `specify init`/`specify integration install codex`, not hand-written. `.specify/memory/constitution.md` (hand-drafted during the earlier migration) was confirmed unchanged by the install.
